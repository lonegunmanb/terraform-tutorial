...
- demo-app-rg —— 一个演示用的 Resource Group
- tfstate-rg —— 用于承载状态存储的 Resource Group
- tfstatelab —— Storage Account，用于后续步骤承载 Terraform 状态
- tfstate —— Storage Account 中的 Blob Container，状态文件将存放在此
...
你应该能看到 tfstate-rg、demo-app-rg 与 tfstatelab Storage Account 都已被创建。
...
在下一步中，我们将把状态迁移到刚刚创建的 tfstatelab Storage Account 的 tfstate Container 中，体验远程后端与 blob lease 状态锁定。
