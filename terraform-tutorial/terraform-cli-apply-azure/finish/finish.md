# 恭喜完成 terraform apply 实战练习（Azure 版）！
...
- 本课程用 [miniblue](https://miniblue.io/)（Docker 镜像版本锁定为 0.7.0）在本地完整模拟 Azure REST API，无需真实订阅
- azurerm provider 通过 metadata_host = "localhost:4567" 指向 miniblue 的 HTTPS 端点
- 所有 apply 子命令、参数、工作流与 AWS / LocalStack 版完全一致——这正是 Terraform 多云抽象的价值所在
