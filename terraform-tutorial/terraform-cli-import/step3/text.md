...
## 也可以用 import 块导入 for_each 资源

除了命令行方式，import 块也支持 for_each 资源。语法如下（仅展示，不需要执行）：

```
cat <<'EOF'
# 等效的 import 块写法
import {
  to = aws_s3_bucket.per_env["dev"]
  id = "app-dev"
}

import {
  to = aws_s3_bucket.per_env["staging"]
  id = "app-staging"
}
EOF
```

两种方式效果相同，选择取决于团队偏好和 CI/CD 流程。
