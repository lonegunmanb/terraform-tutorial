...
```
module.buckets_count[0].aws_s3_bucket.this[0]
module.buckets_count[1].aws_s3_bucket.this[0]
module.buckets_count[2].aws_s3_bucket.this[0]
```
...
动手验证。在 main.tf 末尾添加三个场景的演示代码，使用 S3 模块来观察执行顺序：
...
```bash
head -n 87 main.tf > main.tf.tmp && mv main.tf.tmp main.tf
