...
2. 定义以下 output：
   ...
   - all_bucket_ids: 值为包含三个桶 bucket_id 的列表（顺序：web_assets, api_data, backups）
...
提示：回顾前面几步中 module 块的写法，每个 module 块需要 source、bucket_name 和 tags 三个参数。
...
- web_assets 模块的 bucket_id 和 bucket_arn
- api_data 模块的 bucket_id 和 bucket_arn
- backups 模块的 bucket_id
