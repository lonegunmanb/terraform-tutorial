...
1. 在 modules/storage 目录下创建一个模块，包含三个文件：
   ...
   - main.tf — 创建一个 aws_s3_bucket 资源，bucket 名称使用 var.bucket_name（aws_s3_bucket 的名称参数是 bucket，id 属性是 id，ARN 属性是 arn）
   - outputs.tf — 输出 bucket_id（值为资源的 .id）和 bucket_arn（值为资源的 .arn）
