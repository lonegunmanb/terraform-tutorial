...
terraform console 提供一个交互式 REPL，可实时求值任意 HCL 表达式。进入：
...
由于本课程不会真正 apply azurerm 资源，state 中没有实例数据。但 console 仍能解析配置中的静态属性表达式，例如：

```
azurerm_resource_group.setup.name
```

```
azurerm_resource_group.deploy.location
```
