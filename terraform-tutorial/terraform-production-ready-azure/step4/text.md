...
安全模块封装了 Key Vault 凭证、App Configuration、Managed Identity 和 Role Assignment——三层架构的横切关注点。
...
网络层：vnet_cidr 变量增加了 validation 块，用 can(cidrhost(...)) 确保 CIDR 格式合法。

```bash
diff /root/workspace/modules/web/main.tf /root/stage/step4/modules/web/main.tf
```

Web 层：Load Balancer 增加了 precondition 块，确保至少传入 2 个子网（跨可用区高可用）。
...
数据层：postcondition 块，验证 Cosmos DB 的 offer_type 是 Standard。
...
立刻报错，不需要调用 Azure API。再试一个有效值确认能通过：

```bash
terraform plan -var="vnet_cidr=172.16.0.0/16"
```
...
|------|---------|------------|----------|
| validation | plan 之前 | 仅当前变量 | CIDR 格式、app_name 长度 |
| precondition | apply 之前 | 多个变量、表达式 | LB 子网数 >= 2 |
| postcondition | apply 之后 | self.*（当前资源） | Cosmos DB offer_type |
...
从第一步的 500+ 行单体，到现在不到 150 行的根模块 + 五个职责清晰的子模块。全过程通过 moved 块完成，没有销毁、没有重建一个资源。

```bash
cat moved.tf | grep "moved {" | wc -l
```

总计约 30 个 moved 块，覆盖了所有从单体到五层模块化的资源地址迁移。
