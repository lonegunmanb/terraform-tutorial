...
locals {
  ...
  storage_name_static  = substr(replace("${var.app_name}${var.environment}static${random_string.suffix.result}", "-", ""), 0, 24)
  storage_name_backups = substr(replace("${var.app_name}${var.environment}backups${random_string.suffix.result}", "-", ""), 0, 24)
}
...
resource "azurerm_linux_virtual_machine" "app" {
  ...
  lifecycle {
    ignore_changes = [admin_password, custom_data, identity]
  }
}
