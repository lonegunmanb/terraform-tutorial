...
1. generate 块生成 provider.tf（所有层共用同一个 azurerm provider 配置）
2. inputs 块注入公共变量（suffix、app_name、environment）

本实验把 suffix 固定为 lab，这样 Key Vault 数据面域名在初始化阶段就能预注册到本地 miniblue。
...
- dependency "networking"：读取 vnet_id、subnet_ids
- dependency "security"：读取 app_identity_id
...
每个层只包含自己的资源。改网络层不会刷新 Cosmos DB 的状态，改数据层不需要 VNet 的写锁。
