...
由于 Resource Group / DNS Zone / Virtual Network 的 name 都包含变量（${var.environment}），Terraform 会提出销毁 dev 资源并新建 staging 资源——这体现了 name 变更会触发 replace 的规律。
...
观察 plan 输出，Resource Group、两个 DNS Zone 和 Virtual Network 的名称全部变化，每个资源都会触发 replace（-/+），汇总应为：
