...
输出列出了根模块中声明的所有 output。注意 connection_string 显示为 &lt;sensitive&gt;——因为它在声明时标记了 sensitive = true。
...
```
terraform output all_dns_zones
```
...
前面列出所有 output 时，connection_string 显示为 &lt;sensitive&gt;。但按名称直接查询时，Terraform 会显示实际值：
